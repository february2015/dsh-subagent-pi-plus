 ...
 *
 * @module dsh-subagent-codex-plus/wire
 */
...
function object(value, label) {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        throw new Error(`subagent-codex-plus: app-server returned invalid ${label}`);
    }
    return value;
}
...
function unattendedDecision(params) {
    ...
    throw new Error('subagent-codex-plus: app-server offered no unattended approval decision');
}
...
function abortError(signal) {
    return signal.reason instanceof Error
        ? signal.reason
        : new Error(`subagent-codex-plus: app-server request aborted: ${String(signal.reason)}`);
}
...
export class CodexAppServerWire {
    ...
    async startThread(cwd, signal) {
        ...
        if (thread.ephemeral !== true) {
            throw new Error('subagent-codex-plus: app-server did not create an ephemeral thread');
        }
        this.threadId = id;
    }
    /**
    ...
    async runTurn(texts, signal) {
        ...
        if (status !== 'completed') {
            ...
            throw new Error(`subagent-codex-plus: Codex turn ended with status ${String(status)}${detail}`);
        }
        const output = this.collectOutput();
        if (output.length === 0) {
            this.recordFailure({ stage: 'turn', category: 'invalid-result' });
            throw new Error('subagent-codex-plus: Codex completed without a final answer');
        }
    ...
    onInputEnd = () => {
        ...
        this.fail(new Error('subagent-codex-plus: app-server protocol stream closed'));
    };
    observePendingTurnId(id) {
        if (this.turnCompleted === undefined) {
            throw new Error('subagent-codex-plus: app-server referenced a turn before turn/start');
        }
        if (this.pendingTurnId !== undefined && this.pendingTurnId !== id) {
            throw new Error('subagent-codex-plus: app-server referenced conflicting turns');
        }
        this.pendingTurnId = id;
    }
    commitTurnId(id) {
        if (this.pendingTurnId !== undefined && this.pendingTurnId !== id) {
            throw new Error('subagent-codex-plus: turn/start response did not match the active turn');
        }
        this.turnId = id;
    ...
    validateRunIds(params, nullableTurn = false) {
        if (params.threadId !== this.threadId) {
            throw new Error('subagent-codex-plus: app-server request referenced another thread');
        }
        ...
        if (id !== this.turnId) {
            throw new Error('subagent-codex-plus: app-server request referenced another turn');
        }
        return false;
    }
    ...
    handleServerRequest(method, params) {
        try {
            switch (method) {
                ...
                default:
                    throw new Error(`subagent-codex-plus: unsupported app-server request ${JSON.stringify(method)}`);
            }
        }
        catch (error) {
    ...
    handleNotification(method, params, order) {
        ...
        if (method === 'item/completed') {
            ...
            const text = typeof item.text === 'string'
                ? item.text
                : (() => { throw new Error('subagent-codex-plus: app-server returned an invalid agent message'); })();
            if (item.phase === 'final_answer') {
                ...
                throw new Error(`subagent-codex-plus: app-server returned an unknown agent message phase ${JSON.stringify(item.phase)}`);
            }
            return;
        }
        ...
        if (!['completed', 'interrupted', 'failed'].includes(String(turn.status))) {
            throw new Error(`subagent-codex-plus: app-server returned invalid terminal turn status ${String(turn.status)}`);
        }
